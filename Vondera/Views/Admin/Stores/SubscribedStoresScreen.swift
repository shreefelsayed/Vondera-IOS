...
class SubscribedStoresScreenVM : ObservableObject {
    ...
    func fetch() async {
        ...
        do {
            let result = try await StoresDao().getCurrentlySubscribed(lastSnapshot: lastDoc)
...
struct SubscribedStoresScreen: View {
    @StateObject private var viewModel = SubscribedStoresScreenVM()
    ...
    var body: some View {
        ...
        .navigationTitle("Subscribed Stores")
    }
}
