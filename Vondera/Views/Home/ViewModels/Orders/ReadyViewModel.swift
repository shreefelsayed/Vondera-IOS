...
class ReadyViewModel : ObservableObject {
    ...
    var filteredItems: [Order] {
        ...
        return items.filter { order in
            ...
            || order.phone.localizedCaseInsensitiveContains(searchText)
            || order.gov.localizedCaseInsensitiveContains(searchText)
    ...
    func getOrdersList() async {
        ...
        do {
            let incomeItems = try await ordersDao!.getOrdersByStatue(statue: "Assembled")
            DispatchQueue.main.sync {
